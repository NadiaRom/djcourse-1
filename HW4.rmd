...
air
# нема потреби групувати за тижнем в цьому випадку
week(air$date)
...
df <- df %>%   # не забудьмо присвоїти змінну щоб записати результат
  arrange(date)    # дата вже як дата, as.Date було зайве, хоча на роботу не впливало. Так само ви вже робили раніше селект. Так, це все потенційно можна з'єднати в одному пайплайні, але для навчання простіше розбивати задачу на частини.
```
...
# правильно, але можна спростити:
df <- df %>%
  mutate(year = year(date),
        month = month(date),
        day = day(date))
# lubridate вже є у середдовищі, тож lubridate:: зайве, у мутейті можна писати одразу декілька колонок, щоб не повторювати "mutate" тричі. Це просто стилістика, цей код працює там само, як попередній варіант
...
df <- df %>%
  group_by(day, month) %>%
  mutate(n()) %>%
  filter(2019, 2020) %>%   # не те, дивіться вище
  ungroup(df)
